	...
	return (
		<div className="flex items-center gap-2">
			<Button
				...
				className="relative"
				title={`Previous day (${getShortcutLabel(shortcutKeys.prev)})`}
				aria-label={`Previous day (${getShortcutLabel(shortcutKeys.prev)})`}
			>
				<ChevronLeftIcon />
				<KeyBadge label={shortcutKeys.prev} />
			</Button>
			...
			<Button
				...
				onClick={goToNextDay}
				className="relative"
				title={`Next day (${getShortcutLabel(shortcutKeys.next)})`}
				aria-label={`Next day (${getShortcutLabel(shortcutKeys.next)})`}
			>
				<ChevronRightIcon />
				<KeyBadge label={shortcutKeys.next} />
			</Button>
		</div>
	)
}
