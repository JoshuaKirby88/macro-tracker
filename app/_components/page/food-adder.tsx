...
import { FoodCommand, type FoodCommandHandle } from "@/components/food/food-command"
...
export const FoodAdder = () => {
	...
	const foodCommandRef = useRef<FoodCommandHandle>(null)
	...
	useLeaderShortcut([
		...
		{ key: "s", handler: () => foodCommandRef.current?.open() },
		{ key: "q", handler: focusQuantity, enabled: Boolean(selectedFoodId) },
		{ key: "t", handler: submitCurrentForm, enabled: Boolean(selectedFoodId) },
		...mealShortcuts,
	])
							...
							<Controller
								...
								render={({ field }) => (
									<FoodCommand
										...
										onSelect={() => setTimeout(() => form.setFocus("actualQuantity"), 0)}
									/>
								)}
							/>
						</div>
